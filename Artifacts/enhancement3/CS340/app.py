...
from query_service import (
    ...
    fetch_filtered_animals,
    prepare_dataframe,
)

APP_TITLE = "Dashboard by Dante Nardulli - CS499 Capstone"
...
#Refresh the table using the structured rescue filter pipeline.
@app.callback(Output("datatable-id", "data"), Input("filter-type", "value"))
def update_dashboard(filter_type: str) -> list[dict]:
    if database is None:
        return []
    filtered_df = fetch_filtered_animals(database, filter_type)
    return filtered_df.to_dict("records")


# Render a pie chart using MongoDB aggregation results for the selected filter.
...
def update_graphs(filter_type: str):
    ...
    results = database.aggregate(build_aggregation_pipeline(filter_type))
    if not results:
        return html.Div("No data available to generate chart.")
